...
def get_category(session):
    ...
    try:
        categories = session.query(Categories).order_by(Categories.category_id.asc()).all()
...
def update_category(body: CategoriesUpdateSchema, session):
    ...
    try:
        ...
        if hasattr(category_api, "category_description") and category_api.category_description:
            category.category_description = category_api.category_description
        if hasattr(category_api, "category_type") and category_api.category_type:
            category.category_type = category_api.category_type
